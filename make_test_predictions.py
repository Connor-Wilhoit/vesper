def make_test_predictions(data):
    for idx in tqdm(range(1, len(data.index) - 1)):
        ...
        time_next = row_prev['image_index'].values[0]
        ...
        x1, y1     = preprocess_image_test_from_path(row1['image_path'].values[0], row1['speed'].values[0])
        x2, y2     = preprocess_image_test_from_path(row2['image_path'].values[0], row2['speed'].values[0])
